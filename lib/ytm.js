...
function all(node, key, out = []) {
  ...
  if (Array.isArray(node)) {
    for (const value of node) all(value, key, out);
    return out;
  }
...
function parseItem(renderer) {
  const columns = (renderer.flexColumns || [])
    .map(column => column.musicResponsiveListItemFlexColumnRenderer?.text)
    .filter(Boolean);
  ...
  const fromTitle = titleRun?.navigationEndpoint?.watchEndpoint?.videoId;
  if (!videoId && fromTitle) videoId = fromTitle;

  const overlay = first(renderer.overlay || {}, 'watchEndpoint');
  if (!videoId && overlay?.videoId) videoId = overlay.videoId;
  ...
  const duration = durationRenderer ? label(durationRenderer.text) : null;

  return {
    type: videoId ? 'song' : 'browse',
    title,
    subtitle,
    videoId,
    thumbnail: thumbnail(renderer.thumbnail),
    duration
  };
}
  ...
  return contents.map(section => {
    ...
    if (carousel) {
      const items = (carousel.contents || [])
        .map(entry => entry.musicResponsiveListItemRenderer
          ? parseItem(entry.musicResponsiveListItemRenderer)
    ...
    if (shelf) {
      const items = (shelf.contents || [])
        .map(entry => entry.musicResponsiveListItemRenderer ? parseItem(entry.musicResponsiveListItemRenderer) : null)
        .filter(item => item?.title);
...
function parseTwoRow(renderer) {
  ...
  const title = label(renderer.title);
  const subtitle = label(renderer.subtitle);
  return {
    type: nav.videoId ? 'song' : 'browse',
    title,
    subtitle,
    thumbnail: thumbnail(renderer.thumbnailRenderer || renderer.thumbnail),
    videoId: nav.videoId || null,
    browseId: nav.browseId || null,
    playlistId: nav.playlistId || null
  };
}
...
async function search(query, filter) {
  const body = { query };
  if (SEARCH_FILTERS[filter]) body.params = SEARCH_FILTERS[filter];
  ...
  const items = [];
  const seen = new Set();
  for (const section of all(data, 'itemSectionRenderer')) {
...
async function home() {
  const data = await request('browse', { browseId: 'FEmusic_home' });
  const sectionList = first(data, 'sectionListRenderer');
  return parseSections(sectionList?.contents || []);
}

async function charts() {
  const data = await request('browse', { browseId: 'FEmusic_charts' });
  const sectionList = first(data, 'sectionListRenderer');
  return parseSections(sectionList?.contents || []);
}

async function suggest(query) {
  const data = await request('music/get_search_suggestions', { input: query });
  return all(data, 'searchSuggestionRenderer').map(item => label(item.suggestion)).filter(Boolean);
}

module.exports = { request, search, home, charts, suggest, parseSections };
