...
function parseQueueItem(r) {
  const nav = r.navigationEndpoint?.watchEndpoint || {};
  return {
    type: 'song',
    title: label(r.title),
    subtitle: label(r.shortBylineText),
    videoId: r.videoId || nav.videoId || null,
    playlistId: nav.playlistId || null,
    thumbnail: thumbnail(r.thumbnail),
    duration: label(r.lengthText) || null
  };
}
...
async function next(videoId) {
  ...
  const items = (playlist?.contents || []).map(entry => entry.playlistPanelVideoRenderer ? parseQueueItem(entry.playlistPanelVideoRenderer) : null).filter(x => x?.videoId && x.title);
  const current = items.find(item => item.videoId === videoId) || null;
  const relatedBrowseId = first(data, 'relatedBrowseId') || first(data, 'browseId') || null;
  return {
    current,
    queue: items,
    lyricsBrowseId: first(data, 'browseId') || null,
    relatedBrowseId
  };
}
...
async function song(videoId) {
  const data = await request('next', { videoId });
  const playlist = first(data, 'playlistPanelRenderer') || first(data, 'musicQueueRenderer');
  const items = (playlist?.contents || []).map(entry => entry.playlistPanelVideoRenderer ? parseQueueItem(entry.playlistPanelVideoRenderer) : null).filter(x => x?.videoId);
  const current = items.find(item => item.videoId === videoId);
  if (current) return current;
  return { type: 'song', videoId, title: null, subtitle: null, thumbnail: null, duration: null };
}

module.exports = { request, search, home, charts, suggest, next, related, browse, song, parseSections };
